...
Target "Deploy" (fun _ ->
    ...
    let result =
        ExecProcess (fun info ->
            info.FileName <- "docker"
            info.WorkingDirectory <- deployDir
            info.Arguments <- sprintf "push %s/%s/%s:%s" dockerLoginServer dockerUser dockerImageName release.NugetVersion) TimeSpan.MaxValue
    ...
    let result =
        ExecProcess (fun info ->
            info.FileName <- "docker"
            info.WorkingDirectory <- deployDir
            info.Arguments <- sprintf "push %s/%s/%s:latest" dockerLoginServer dockerUser dockerImageName) TimeSpan.MaxValue
    if result <> 0 then failwith "Docker push failed"
)
